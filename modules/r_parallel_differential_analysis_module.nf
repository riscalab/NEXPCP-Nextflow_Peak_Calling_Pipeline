process find_diff_peaks_R_process {
    ...
    // commenting out the conda environment for R because now I am using apptainer. check nextflow.config to see
    ...
    if (params.narrowPeak_data || params.sicer2) {
        ...
        ## here just make a copy of the countsMatrix
        #countsMatrix2 = countsMatrix
        # or just try to mank the rownames after the parallel function runs
        ...
        # getting the foreach, future, futurize, libraries to parallelize 
        library(foreach)
        library(future)
        library(futurize)
        library(doFuture)
        
        # need to register the do future
        registerDoFuture()
        
        #then use plan multisession for parallelization
        plan(multisession)


        countsMatrix2 = foreach (x = 1:length(list_of_mBams), .combine = cbind) %dopar% {
        ...
        # now I need to return the fragment counts
        # and save the foreach return data as a variable called countsMatrix2
        ...
        # I can probably vectorize getting the base names of the bam files
        colnames(countsMatrix2) = basename(list_of_mBams)

        # here i need to get the condition and type for later, since I cant do that in the for loop anymore
        ...
        keep_Rows = which(rowSums(countsMatrix2) > 5)

        filt_countmatrix = countsMatrix2[keep_Rows,]

        # now to get the condition_design
        #condition_counts = table(unlist(condition_design))

        # new way to get the counts
        ...
        # this is acutally where i get the condition factor
        ...
        # this would be the updated version that wasnt being used anymore
        #type_counts = factor(unlist(condition_type_lists[2,]))
        ...
        # this would be where I get the real type factor
        ...
        # this caused the process to run 2 times longer, mainly because the time it takes to load everything in the futurize() call
